      ...
      <nav className='flex items-center justify-between max-w-screen-xl mx-auto h-full px-4 font-ui-sans-serif'>
        ...
        >
          <div className='flex flex-col absolute right-5 gap-1 items-center'>
            <section
              className={` w-7 rounded h-1 origin-left transition-transform duration-300  bg-[#008bd0] ${isOpen ? 'rotate-[45deg]' : 'rotate-0'
                }`}
            ></section>
            <section
              className={` w-7 rounded h-1  bg-[#008bd0] ${isOpen ? 'opacity-0' : 'opacity-100'
                }`}
            ></section>
            <section
              className={` w-7 rounded h-1 origin-left transition-transform duration-300  bg-[#008bd0] ${isOpen
        ...
        {/* Navigation Items */}
        <div
          className={`flex-grow flex items-center ${isOpen ? 'h-screen' : ''
            } md:flex md:items-center md:justify-center md:h-auto`}
        >
          <ul
            ref={navRef}
            className={`${isOpen
                ? 'absolute top-20 right-0 w-1/2 h-screen bg-gradient-to-br from-[#61dafb] to-blue-500'
                : 'absolute top-[-100vh] right-0 w-1/2 h-screen'
              } 
